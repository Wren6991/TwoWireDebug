...
The downstream bus supports (some subset of) 8- to 64-bit aligned reads and writes, with a fixed address width between 8 and 64 bits. Further details of the bus are implementation-defined, and should be invisible to the DTM. Any address-mapped hardware could be attached, such as: a RISC-V Debug Module, CoreSight Access Ports, or direct system bus access.
...
|`0x7` |<<cmd-r.data>>    | Perform bus read, and get result of last `R.DATA`  | 1-8 bytes read
|`0x8` |<<cmd-r.buff>>    | Get result of last `R.DATA`                        | 1-8 bytes read
|`0x9` |<<cmd-w.data>>    | Perform bus write                                  | 1-8 bytes write
|`0xb` |<<cmd-r.ainfo>>   | Read from the <<address-info-table>>               | 4 bytes read
...
Initiate a downstream bus read from the address indicated by <<reg-addr>>, and return the result of the last completed downstream bus read. The data size of the bus read, and the payload size of the `R.DATA` command, are determined by the <<reg-csr>>.`DSIZE` field.
...
* <<reg-csr>>.`DSIZE` has been changed since the last `R.DATA`
...
Initiate a downstream bus write to the address indicated by <<reg-addr>>. The data size of the bus write, and the payload size of the `W.DATA` command, are determined by the <<reg-csr>>.`DSIZE` field.
...
| 21:20 | `DSIZE`        | Data transfer size (write-any read-legal). Size in bits = 8 << `DSIZE`.

The DTM may not support all values of `DSIZE`. If the host writes an unsupported value, the DTM sets `DSIZE` to some implementation-defined value which it _does_ support. The host can discover the supported data sizes by writing and reading back `DSIZE`.
...
This register contains an 8- to 64-bit address, indicating the memory location to be accessed by <<cmd-w.data>> and <<cmd-r.data>> commands.

===== Address Scaling

The addressing unit of `ADDR` is determined by the current value of <<reg-csr>>.`DSIZE`:

* `DSIZE` = 0: address is in units of 1 byte
* `DSIZE` = 1: address is in units of 2 bytes
* `DSIZE` = 2: address is in units of 4 bytes
* `DSIZE` = 3: address is in units of 8 bytes

For example, a DTM attached to a RISC-V Debug Module, requiring only 32-bit transfers with a 7-bit word address, could have an `ASIZE` of 0 (8-bit addresses) and have `DSIZE` hardwired to 2. This allows the DTM to access arbitrary 8-bit word addresses or, equivalently, all word-aligned locations in a 10-bit byte-addressed space.

Or, a target which exposes a 32-bit system address space alongside some debug-specific registers could have an `ASIZE` of 3, such that:

* Byte read/writes are supported on the entire 32-bit byte-addressed system bus (4 GiB of address space)
* Word read/writes can see a larger 16 GiB address window
** Debug-specific registers mapped at a +4GiB offset, accessible only from the DTM
** Could provide different views of memory, e.g. physical vs virtual
...
This table doesn't try to replace any of the standard discovery mechanisms that may be used downstream (such as a CoreSight ROM table). Rather, it should give enough information to identify what those standard mechanisms _are_, and where to start the discovery process.
...
|===
| Bits | Name | Description
| 31:24 | `BASE` | Segment base address, corresponding to the 8 MSBs of the <<reg-addr>> value, at the largest supported <<reg-csr>>.`DSIZE`.
| 22:16 | `SIZE` | Log2 of segment size in bytes.
...
| 1 | `PRESENT` | If 0, the hardware corresponding to this entry is not present (e.g. has been tied off). This entry may still be `VALID`, in which case, the host should continue reading the table..
| 0 | `VALID` | If 1, this is a valid AINFO table entry. If 0, there is no AINFO information in this entry, and the host should not read past this point.
|===
