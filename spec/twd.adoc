...
Throughout this document, _target_ refers to the device under test: an integrated system with a single DTM, some address-mapped debug components on a bus downstream of the DTM, and presumably some other hardware to be debugged. Conversely, _host_ refers to the system communicating with and commanding the DTM, e.g. a developer's laptop and a USB debug probe.
...
** An abbreviated status read <<cmd-r.stat>> makes status polling more efficient for simple hosts.
* Multidrop bus, with a standard method of setting 4-bit multidrop address.

The downstream bus supports 32-bit aligned reads and writes. The address width is a multiple of 8 bits, between 8 and 64. All other details are implementation-defined. Any hardware can be attached, such as: a RISC-V Debug Module, CoreSight Access Ports, or direct system bus access.
...
|`0x2` |<<cmd-r.ainfo>>   | Read the <<address-info-table>>                    | 4 bytes read
|`0x4` |<<cmd-r.stat>>    | Read abbreviated status                            | 1 byte combined read + parity
|`0x6` |<<cmd-w.csr>>     | Write control/status register                      | 4 bytes write
|`0x7` |<<cmd-r.csr>>     | Read control/status register                       | 4 bytes read
|`0x8` |<<cmd-r.addr>>    | Read address register                              | 1-8 bytes read
|`0x9` |<<cmd-w.addr>>    | Write address register                             | 1-8 bytes write
|`0xa` |<<cmd-w.addr.r>>  | Write address register and perform bus read        | 1-8 bytes write
|`0xb` |<<cmd-r.data>>    | Perform bus read, and get result of last bus read  | 4 bytes read
|`0xc` |<<cmd-w.data>>    | Perform bus write                                  | 4 bytes write
|`0xd` |<<cmd-r.buff>>    | Get result of last bus rad                         | 4 bytes read
...
Returns a short response containing just the error and status flags from the CSR, in a single byte. This improves performance for simple hosts that do not implement batching and prefer to poll after every transfer.

The response is abbreviated, consisting of four flags followed by a parity bit, a stop bit and two turnaround bits (the non-optional part of a parity byte).

In serial wire order the flags are:
...
This is effectively a four-bit payload followed by a parity byte. Since the last four bits of the parity byte are optional, this does not break the overall byte-aligned structure of TWD.
...
Write to the address register, like <<cmd-w.addr>>. After writing to the address register, initiate a bus read from the written address.

This command avoids an unnecessary data transfer at the beginning of a linear sequence of `R.DATA` commands, and improves the throughput of random read accesses.

If any error flag is set in the <<reg-csr>>, `W.ADDR.R` is ignored. If a downstream bus access is still in progress at the point a `W.ADDR.R` is issued, the <<reg-csr>>.`EBUSY` flag is set, and <<reg-addr>> is not written.
