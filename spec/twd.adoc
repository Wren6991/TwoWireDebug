...
Returns a short response containing just the error and status flags from the CSR, in a single byte. This improves performance for simple hosts that do not implement and prefer to poll after every transfer.
